...
#define PAD_CLEAR_HORIZONTAL(p) do{(p)->current_state.left=0;(p)->current_state.right=0;}while(0)
#define PAD_CLEAR_VERTICAL(p) do{(p)->current_state.up=0;(p)->current_state.down=0;}while(0)
